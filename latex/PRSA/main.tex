\documentclass[openacc]{rsproca_new}%%%% Royal Society Proc. R. Soc. A class
...
%%%% Packages required by the article body. amsmath/amssymb/amsfonts/amsthm and
%%%% graphicx are already loaded by the class; tikz and float are added for the
%%%% figures, and graphicspath points at the figures subdirectory.
\graphicspath{{figs/}}
\usepackage{float}
\usepackage{tikz}

%%%% The class loads `color'; TikZ then loads `xcolor' on top of it. Re-assert the
%%%% class colour definitions so that xcolor registers them in its own model,
%%%% which silences the spurious "Incompatible color definition" warnings.
\definecolor{gray}{cmyk}{0,0,0,.2}
\definecolor{jobcolor}{cmyk}{1,.10,0,.10}
\definecolor{joblightcolor}{cmyk}{0.10,0.068,0,0.012}
\definecolor{abstractcolor}{cmyk}{.04,.04,.12,.08}
...
\author{Davide Batic$^{1}$ and Denys Dutykh$^{1}$}

\address{$^{1}$Mathematics Department, Khalifa University of Science and Technology, PO Box 127788, Abu Dhabi, United Arab Emirates}

\subject{mathematical physics, electromagnetism}

\keywords{T-duality, zero-point length, Green function, planar electrostatics, finite-size effects, dimensional flow}

\corres{Davide Batic\\
\email{davide.batic@ku.ac.ae}}

\begin{abstract}
We develop a finite-size formulation of T-duality-deformed electrostatics in $(2+1)$ dimensions. The T-duality form factor regularises the ultraviolet behaviour of the planar static Green function, while finite volume and boundary conditions control the infrared sector. We first rederive the self-energy-subtracted infinite-plane interaction, which is finite at the origin and recovers the logarithmic Coulomb law at large distances.\absbreak We then define the corresponding Green function on general domains via the spectral calculus of the Laplacian and work out explicit realisations on the square torus, the grounded half-plane, and the grounded strip. The torus implements charge neutrality through zero-mode subtraction, whereas Dirichlet boundaries produce finite image and self-energy effects. Finally, we introduce an additive-constant-free running interaction dimension and show that the ultraviolet flow to one dimension survives in finite volume, while the infrared behaviour is geometry-dependent.
\end{abstract}
